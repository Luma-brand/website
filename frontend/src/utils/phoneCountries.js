...
function getFlagEmoji(iso2 = "US") {
  const code = String(iso2 || "US").toUpperCase();
...
export function getCountryOptions() {
  ...
  return getRegionCodes()
    .map((iso2) => ({
      ...
      callingCode: COUNTRY_CALLING_CODES[iso2] || "",
    }))
    .sort((a, b) => a.name.localeCompare(b.name));
}

export function getCountryByIso2(iso2 = "US") {
  return (
    getCountryOptions().find(
      (country) => country.iso2.toUpperCase() === String(iso2).toUpperCase()
    ) || getCountryOptions().find((country) => country.iso2 === "US")
  );
}
...
export function buildE164Phone(localNumber = "", country = getCountryByIso2("US")) {
  ...
  const nationalNumber =
    country?.iso2 === "NG" && cleaned.startsWith("0")
      ? cleaned.slice(1)
      : cleaned.replace(/^0+/, "");

  return country?.callingCode ? `+${country.callingCode}${nationalNumber}` : cleaned;
}
