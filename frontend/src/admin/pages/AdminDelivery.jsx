...
const emptyLocation = {
  provider: "GIG_LOGISTICS", state: "", city: "", area: "",
  branchName: "", fullAddress: "", latitude: "", longitude: "",
  active: true, lastVerifiedAt: "",
};
...
export function AdminDelivery() {
  ...
  const loadData = useCallback(async () => {
    try {
      setIsLoading(true);
      setError("");
      const [deliveryResponse, locationResponse] = await Promise.all([
        getDeliveryOverview(), getPickupLocations({}, { admin: true }),
      ]);
  ...
  const filteredLocations = useMemo(() => {
    ...
    return locations.filter((location) => !query ||
      [location.state, location.city, location.area, location.branch_name, location.full_address]
        .some((value) => String(value || "").toLowerCase().includes(query)));
  ...
  function saveLocation(event) {
    ...
    }, editingLocationId ? "Pickup branch updated." : "Pickup branch added.");
  }
  ...
  function toggleLocation(location) {
    ...
    }), location.active ? "Pickup branch deactivated." : "Pickup branch reactivated.");
  }
  ...
  return (
    <>
      <AdminTopbar title="Delivery" subtitle="Automated Nigerian routes, home delivery, and GIG Logistics pickup locations." />
      <section className="admin-content delivery-engine-admin">
        ...
        <div className="admin-metric-grid">
          ...
          <div className="admin-card"><span>Active pickup branches</span><strong>{status.active_pickup_count || 0}</strong></div>
        </div>
        ...
        <form className="admin-card pickup-location-form" onSubmit={saveLocation}>
          <div className="admin-section-heading"><div><h2>{editingLocationId ? "Edit pickup branch" : "Add pickup branch"}</h2><p>Deactivate old branches instead of deleting historical references.</p></div><MapPin size={20} /></div>
          ...
          <div className="admin-actions-row"><button className="admin-button" type="submit"><Save size={16} /> {editingLocationId ? "Update branch" : "Add branch"}</button>{editingLocationId && <button className="admin-button secondary" type="button" onClick={() => { setEditingLocationId(""); setLocationForm(emptyLocation); }}>Cancel</button>}</div>
        </form>

        <div className="admin-card admin-table-card">
          <div className="admin-table-header"><div><h2>Pickup locations</h2><p>{locations.length} locally stored branches.</p></div><label className="admin-search"><Search size={16} /><input value={locationSearch} onChange={(event) => setLocationSearch(event.target.value)} placeholder="Search branch" /></label></div>
          <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Branch</th><th>State / city</th><th>Address</th><th>Status</th><th>Action</th></tr></thead><tbody>{filteredLocations.map((location) => <tr key={location.id}><td><strong>{location.branch_name}</strong></td><td>{location.state}<small>{location.city}</small></td><td>{location.full_address}</td><td><span className={`admin-badge ${location.active ? "success" : "warning"}`}>{location.active ? "Active" : "Inactive"}</span></td><td><button type="button" className="admin-button small secondary" onClick={() => editLocation(location)}>Edit</button><button type="button" className="admin-button small" onClick={() => toggleLocation(location)}>{location.active ? "Deactivate" : "Activate"}</button></td></tr>)}</tbody></table></div>
        </div>
