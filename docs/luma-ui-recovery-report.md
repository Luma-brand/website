...
- Expected reduction: about 56–60%. Final rendered height is recorded during preview-browser verification.
...
| --- | ---: | ---: |
| Initial JavaScript bundle | 1,144.16 KB | 421.56 KB |
| Initial JavaScript gzip | 322.76 KB | 133.10 KB |
| Initial JS reduction | — | 63.2% raw / 58.8% gzip |
| Vite build | 1.52 s on current Vercel production | 0.88 s locally |
...
The browser-visible delay is not explained by the product SQL. Live requests show variable first-byte times and occasional gateway failures from the current hosting path, while the database query itself is fast. Render service metrics and cold-start logs still require the production workspace selection before a final infrastructure conclusion.
...
## 16. External credentials and approvals still required

- Explicit approval to apply Neon migration `f6f64832-6a74-4235-a5b1-b82e0ec723f7` for admin-driven promotions.
- Explicit approval to apply Neon migration `5ea2a2c7-1240-4d10-abbd-5a8929875623` for delivery architecture.
- Confirmation that Render workspace `Igiehon's workspace` is the correct production workspace.
...
- Final Render cold-start metrics, backend deployment, and production end-to-end payment verification are blocked until the Render workspace is confirmed.
- Database changes are verified on Neon temporary branches but are not applied to the main branch without explicit approval.
- International weight/value tiers are intentionally not activated until LUMA approves researched carrier rate cards.
- Exact rendered after-height and screenshot evidence require the safe preview deployment/browser pass.
